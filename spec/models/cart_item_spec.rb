...
RSpec.describe CartItem, type: :model do
	...
	it "單一訂單總金額未超過2000元時, 運費收150元" do
		expect(@cart.shipping).to be 150
	end
	it "單一訂單總金額超過2000元免運費" do
		c2 = Category.create(title: "discount")
		p3 = Product.create(title: "Product 3", price: 1000, category_id: c2.id)
		@cart.add_item(p3.id)

		expect(@cart.shipping).to be 0
	end
	it "計算加運費後的總金額" do
